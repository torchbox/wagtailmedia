...
def sniff_media_data(video_path):
    ...
    p = subprocess.check_output([ffprobe, "-v", "quiet", "-print_format", "json", "-show_format", "-show_streams", video_path])
...
def generate_media_thumb(video_path, out_path, skip_seconds=0):
    ...
    subprocess.check_output([ffmpeg, "-y", "-v", "quiet", "-accurate_seek", "-ss", str(skip_seconds), "-i", video_path, "-frames:v", "1", out_path])
    return out_path
