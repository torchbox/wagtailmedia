...
if WAGTAIL_VERSION >= (8, 0):
    ...
    class TestV3MediaListing(TestV3MediaBase):
        ...
        def test_anonymous_can_list(self):
            ...
            self.assertEqual(self.listed_ids(response), [media.id])

        def test_meta_fields_and_absolute_urls(self):
            media_item = self.create_media()
            ...
            self.assertTrue(
                item["meta"]["detail_url"].endswith(
                    reverse(
                        ...
                        kwargs={"media_id": item["id"]},
                    )
                )
            )
            self.assertIn(media_item.file.url, item["meta"]["download_url"])

        def test_search_by_title(self):
            self.create_media(title="Wagtail guide")
            self.create_media(title="Django guide")
        ...
        def test_order_by_title(self):
            self.create_media(title="Beta")
            self.create_media(title="Alpha")
        ...
        def test_filter_by_title(self):
            media = self.create_media(title="Only this")
            self.create_media(title="Not this")
            response = self.get_response(title="Only this")
            self.assertEqual(self.listed_ids(response), [media.id])

        def test_listing_prefetches_tags(self):
            for index in range(3):
                media = self.create_media(title=f"Media {index}")
                media.tags.add("one", "two")
        ...
        def test_direct_login_restriction_excludes_anonymous(self):
            ...
            self.assertNotIn(media.id, self.listed_ids(self.get_response()))
        ...
        def test_direct_login_restriction_accepts_bearer_user(self):
            ...
            self.login()
            self.assertIn(media.id, self.listed_ids(self.get_response()))

        def test_direct_group_restriction_checks_bearer_user_groups(self):
            collection = self.create_collection("Group only")
            media = self.create_media(collection=collection)
            allowed_group = Group.objects.create(name="allowed media readers")
        ...
        def test_ancestor_restriction_hides_descendant_media(self):
